...
export const ManageBuyers: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <Breadcrumb items={[{ label: 'Manage Buyers' }]} />

      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">Commercial Buyer Directory</h1>
          <p className="text-xs sm:text-sm text-slate-500">
            Monitor wholesale distributors, retail supermarkets, export houses, and procurement history.
          </p>
        </div>

        <div className="text-xs font-bold text-slate-600 bg-white px-4 py-2 rounded-2xl border border-slate-200 shadow-soft">
          Active Buyers: <strong className="text-amber-700">{buyers.length}</strong>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-soft">
        <div className="relative max-w-md">
          ...
          <input
            ...
            placeholder="Search company name, buyer contact, or city..."
          ...
          return (
            ...
            >
              <div>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-500">{buyer.businessName || 'Wholesale Buyer'}</p>
                    </div>
                  </div>
                ...
                <div className="p-3 bg-amber-50/50 rounded-2xl border border-amber-100 text-xs text-slate-700 space-y-1.5 mt-4">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-slate-900">{orders.length} Consignments</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">Total Purchase GMV:</span>
                    <span className="font-black text-amber-800">₹{totalSpent.toLocaleString('en-IN')}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">HQ City:</span>
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                <span className="text-[11px] text-slate-400">Status: <strong className="text-emerald-600">Active</strong></span>
                <button
      ...
      {selectedBuyer && (
        <Modal
          ...
          title={`Buyer Profile: ${selectedBuyer.name}`}
          subtitle={`Company: ${selectedBuyer.businessName} • Type: ${selectedBuyer.buyerType}`}
        >
          <div className="space-y-4 text-xs">
            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-2">
              <div className="font-bold text-slate-900">Commercial Contact Details</div>
              <p className="text-slate-600">Email: {selectedBuyer.email}</p>
              <p className="text-slate-600">Phone: {selectedBuyer.phone}</p>
              <p className="text-slate-600">Registered City: {selectedBuyer.location}</p>
            </div>
